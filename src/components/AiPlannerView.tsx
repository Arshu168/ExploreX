              ...
              <div className="space-y-3 relative pl-4 border-l-2 border-blue-200 dark:border-blue-900">
                {day.activities.map((act) => (
                  <div key={act.id} className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 space-y-2 shadow-2xs hover:shadow-xs transition relative">
                    ...
                    {/* DOWNSIDE LOCATION DETAILS & GOOGLE MAPS OPEN LINK */}
                    {act.locationName && (
                      <div className="pt-2.5 mt-2 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2.5 bg-slate-50/70 dark:bg-slate-800/40 p-2.5 rounded-xl">
                        <div className="flex items-center gap-1.5 text-slate-700 dark:text-slate-300 text-xs font-semibold flex-1 min-w-[200px]">
                          <MapPin className="w-3.5 h-3.5 text-rose-500 shrink-0" />
                          <span className="text-[11px] font-medium leading-tight">{act.locationName}</span>
                        </div>

                        <div className="flex items-center gap-1.5 shrink-0">
                          <a
                            href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(act.locationName + ' ' + act.title)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold flex items-center gap-1.5 transition shadow-xs cursor-pointer"
                          ...
                          <a
                            href={`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(tripSummary.originLocation)}&destination=${encodeURIComponent(act.locationName + ' ' + act.title)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-2.5 py-1.5 rounded-xl bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 text-[11px] font-bold flex items-center gap-1 transition cursor-pointer"
                            title={`Get directions from ${tripSummary.originLocation}`}
                          >
                            <span>🧭 Directions (from {tripSummary.originLocation})</span>
                          </a>
                        </div>
                      </div>
                    )}
